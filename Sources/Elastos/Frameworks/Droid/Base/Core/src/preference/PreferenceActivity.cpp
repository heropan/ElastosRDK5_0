    ...
    /* [in] */ IMessage* msg)
{
    ...
    switch(what) {
        ...
        case MSG_BUILD_HEADERS:
            ...
            mHost->OnBuildHeaders(container);
            AutoPtr<IBaseAdapter> adapter = (IBaseAdapter*)mHost->mAdapter->Probe(EIID_IBaseAdapter);
            if (adapter != NULL) {
...
CAR_INTERFACE_IMPL(PreferenceActivity::BackButtonListener, IViewOnClickListener);
...
CAR_INTERFACE_IMPL(PreferenceActivity::SkipButtonListener, IViewOnClickListener);
...
CAR_INTERFACE_IMPL(PreferenceActivity::NextButtonListener, IViewOnClickListener)
...
PInterface PreferenceActivity::Probe(
    /* [in] */ REIID riid)
{
    if (riid == EIID_IPreferenceActivity) {
        return (IPreferenceActivity*)this;
    }
    else if (riid == EIID_IPreferenceManagerOnPreferenceTreeClickListener) {
        return (IPreferenceManagerOnPreferenceTreeClickListener*)this;
    }
    else if (riid == EIID_IPreferenceFragmentOnPreferenceStartFragmentCallback) {
        return (IPreferenceFragmentOnPreferenceStartFragmentCallback*)this;
    }

    return ListActivity::Probe(riid);
}

UInt32 PreferenceActivity::AddRef()
{
    return ElRefBase::AddRef();
}

UInt32 PreferenceActivity::Release()
{
    return ElRefBase::Release();
}

ECode PreferenceActivity::GetInterfaceID(
    ...
    /* [out] */ InterfaceID *pIID)
{
    ...
    if (pObject == (IInterface *)(IPreferenceActivity *)this) {
        *pIID = EIID_IPreferenceActivity;
        return NOERROR;
    }
    else if (pObject == (IInterface *)(IPreferenceManagerOnPreferenceTreeClickListener *)this) {
        *pIID = EIID_IPreferenceManagerOnPreferenceTreeClickListener;
        return NOERROR;
    }
    else if (pObject == (IInterface *)(IPreferenceFragmentOnPreferenceStartFragmentCallback *)this) {
        *pIID = EIID_IPreferenceFragmentOnPreferenceStartFragmentCallback;
        return NOERROR;
    }
    ...
    /* [in] */ IBundle* savedInstanceState)
{
    ...
    else if (mHeaders.Begin() != mHeaders.End()) {
        AutoPtr<IObjectContainer> container;
        CObjectContainer::New((IObjectContainer**)&container);
        List<AutoPtr<IPreferenceActivityHeader> >::Iterator ator = mHeaders.Begin();
        for (; ator != mHeaders.End(); ++ator) {
            container->Add(*ator);
        }
        AutoPtr<IArrayAdapter> adapter;
        CPreferenceActivityHeaderAdapter::New(THIS_PROBE(IContext), container, (IArrayAdapter**)&adapter);
    ...
    /* [in] */ IPreferenceActivityHeader* header)
{
    if (mCurHeader.Get() == header) {
        ...
        List<AutoPtr<IPreferenceActivityHeader> >::Iterator itr = mHeaders.Begin();
        for (; itr != mHeaders.End(); ++index, ++itr) {
            if ((*itr).Get() == header) break;
        }

        Int32 curIndex = 0;
        List<AutoPtr<IPreferenceActivityHeader> >::Iterator curItr = mHeaders.Begin();
        for (; curItr != mHeaders.End(); ++curIndex, ++curItr) {
            if (*curItr == mCurHeader) break;
        }

        Int32 direction = index - curIndex;
        String fragment;
        header->GetFragment(&fragment);
        AutoPtr<IBundle> fragmentArguments;
        header->GetFragmentArguments((IBundle**)&fragmentArguments);
        SwitchToHeaderInner(fragment, fragmentArguments, direction);
