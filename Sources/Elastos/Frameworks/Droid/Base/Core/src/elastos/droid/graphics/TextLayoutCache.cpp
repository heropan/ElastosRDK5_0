...
#include <skia/ports/SkTypeface_android.h>
...
sp<TextLayoutValue> TextLayoutCache::getValue(const SkPaint* paint,
            const Char16* text, Int32 start, Int32 count, Int32 contextCount, Int32 dirFlags)
{
    AutoMutex _l(mLock);
    nsecs_t startTime = 0;
    if (mDebugEnabled) {
...
{
    return sizeof(TextLayoutValue) + sizeof(Float) * mAdvances.capacity() +
            sizeof(Char16) * mGlyphs.capacity() + sizeof(Float) * mPos.capacity();
}
...
void TextLayoutShaper::computeValues(const SkPaint* paint, const UChar* chars,
        size_t start, size_t count, size_t contextCount, int dirFlags,
        Vector<Float>* const outAdvances, Float* outTotalAdvance,
        Vector<Char16>* const outGlyphs, Vector<Float>* const outPos)
{
        *outTotalAdvance = 0;
        if (!count) {
            return;
        ...
        Vector<Char16>* const outGlyphs, Vector<Float>* const outPos)
{
    ...
    Float totalAdvance = *outTotalAdvance;
    while ((isRTL) ?
        ...
        if (outGlyphs) {
            ...
            for (size_t i = 0; i < countGlyphs; i++) {
                Char16 glyph = glyphBaseCount +
                        (Char16) mShaperItem.glyphs[(!isRTL) ? i : countGlyphs - 1 - i];
#if DEBUG_GLYPHS
...
sp<TextLayoutValue> TextLayoutEngine::getValue(const SkPaint* paint, const Char16* text,
        Int32 start, Int32 count, Int32 contextCount, Int32 dirFlags)
{
