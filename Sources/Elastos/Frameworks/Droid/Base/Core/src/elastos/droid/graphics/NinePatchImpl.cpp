...
#include <skia/utils/SkNinePatch.h>
        ...
        const SkPaint* paint, SkRegion** outRegion)
{
    ...
    if (canvas) {
        ...
        LOGV(LOG_TAG, "ninepatch [%g %g %g] [%g %g %g]\n",
...
#ifdef USE_TRACE
    LOGV(LOG_TAG, "NinePatch [%d %d] bounds [%g %g %g %g] divs [%d %d]\n",
             bitmap.width(), bitmap.height(),
    ...
    for (j = yIsStretchable ? 1 : 0;
        ...
        for (i = xIsStretchable ? 1 : 0;
            ...
            if (canvas) {
#ifdef USE_TRACE
                LOGV(LOG_TAG, "-- src [%d %d %d %d] dst [%g %g %g %g]\n",
