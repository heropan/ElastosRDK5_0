...
const UInt32 IconUtilities::sColors[3] = { 0xffff0000, 0xff00ff00, 0xff0000ff };
    ...
    , mColorIndex(0)
{
    ...
    IMaskFilter* mf = IMaskFilter::Probe(bmf);
    mBlurPaint->SetMaskFilter(mf);
    ...
    IMaskFilter* filtermf = IMaskFilter::Probe(filter);
    mGlowColorPressedPaint->SetMaskFilter(filtermf);
    ...
    mGlowColorFocusedPaint->SetMaskFilter(filtermf);
    ...
    IDrawFilter* drawFilter = IDrawFilter::Probe(pfdFilter);
    ...
    /* [in] */ IDrawable* src)
{
    ...
    IDrawable* pbd = IDrawable::Probe(pressedBD);
    IDrawable* fbd = IDrawable::Probe(focusedBD);
    IDrawable* sbd = IDrawable::Probe(scaledBD);
    result->AddState(focusedArray, pbd);
    result->AddState(pressedArray, fbd);
    result->AddState(scaledArray, sbd);

    IDrawable* resDB = IDrawable::Probe(result);
    resDB->SetBounds(0, 0, mIconTextureWidth, mIconTextureHeight);
    return resDB;
}
    ...
    /* [in] */ IDrawable* icon)
{
    ...
    if (IPaintDrawable::Probe(icon) != NULL) {
        ...
        IShapeDrawable* sdb = IShapeDrawable::Probe(painter);
        sdb->SetIntrinsicWidth(width);
        sdb->SetIntrinsicHeight(height);
    }
    ...
    /* [in] */ Boolean pressed)
{
    ...
    dest->DrawBitmap(src, 0.0, 0.0, mPaint);
    dest->SetBitmap(NULL);
