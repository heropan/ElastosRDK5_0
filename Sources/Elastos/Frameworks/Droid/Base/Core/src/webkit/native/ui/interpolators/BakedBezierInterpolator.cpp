...
AutoPtr< ArrayOf<Float> > BakedBezierInterpolator::FADE_OUT_VALUES = BakedBezierInterpolator::InitFadeOutValus();

AutoPtr< ArrayOf<Float> > BakedBezierInterpolator::FADE_IN_VALUES = BakedBezierInterpolator::InitFadeInValus();
    ...
    /* in */ Float input)
{
    ...
    Int32 position = Math::Min(
            (Int32)(input * (mValues->GetLength() - 1)),
            mValues->GetLength() - 2);
