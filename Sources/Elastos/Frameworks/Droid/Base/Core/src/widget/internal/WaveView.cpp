...
CAR_INTERFACE_IMPL(WaveView::LockTimerRunnable, IRunnable)
CAR_INTERFACE_IMPL(WaveView::AddWaveRunnable, IRunnable)

WaveView::LockTimerRunnable::LockTimerRunnable(
    /* [in] */ WaveView* host) : mHost(host)
{}

ECode WaveView::LockTimerRunnable::Run()
{
    if (DBG) Logger::V(TAG, "LockTimerActions");
    // reset lock after inactivity
    if (mHost->mLockState == STATE_ATTEMPTING) {
        if (DBG) Logger::V(TAG, "Timer resets to STATE_RESET_LOCK");
        mHost->mLockState = STATE_RESET_LOCK;
    }
    // for prototype, reset after successful unlock
    if (mHost->mLockState == STATE_UNLOCK_SUCCESS) {
        if (DBG) Logger::V(TAG, "Timer resets to STATE_RESET_LOCK after success");
        mHost->mLockState = STATE_RESET_LOCK;
    }
    mHost->Invalidate();
    return NOERROR;
...
WaveView::AddWaveRunnable::AddWaveRunnable(
    /* [in] */ WaveView* host) : mHost(host)
{}

ECode WaveView::AddWaveRunnable::Run()
{
    String strX("x");
    String strY("y");
    String alpha("alpha");
    ...
    if (mHost->mFinishWaves) {
        ...
        mHost->PostDelayed(mHost->mAddWaveAction, mHost->mWaveTimerDelay);
    }
    return NOERROR;
}
...
{
    assert(0 && "TODO");
//    CDrawableHolder::New(CreateDrawable(R::drawable::unlock_ring), &mUnlockRing);
    ...
    assert(0 && "TODO");
//    CDrawableHolder::New(CreateDrawable(R::drawable::unlock_default), &mUnlockDefault);
    ...
    assert(0 && "TODO");
//    CDrawableHolder::New(CreateDrawable(R::drawable::unlock_halo), &mUnlockHalo);
    ...
    /* [in] */ IMotionEvent* event)
{
    ...
    Invalidate();
    return handled ? TRUE : View::OnTouchEvent(event);
}
...
void WaveView::AnnounceUnlockHandle()
{
    ...
    CStringWrapper::New(description, (ICharSequence**)&cDescription);
    ...
    /* [in] */ Int32 whichHandle)
{
    ...
    if (mOnTriggerListener != NULL) {
        mOnTriggerListener->OnTrigger((IView*)this->Probe(EIID_IView), whichHandle);
    }
}
    ...
    /* [in] */ Int32 newState)
{
    if (newState != mGrabbedState) {
        ...
        if (mOnTriggerListener != NULL) {
            mOnTriggerListener->OnGrabbedStateChange((IView*)this->Probe(EIID_IView), mGrabbedState);
        }
    }
}
...
{
    if (DBG) Logger::V(TAG, "reset() : resets state to STATE_RESET_LOCK");
    mLockState = STATE_RESET_LOCK;
    Invalidate();
