    ...
    /* [in] */ Int32 width)
{
    ...
    Int32 curNumRows =
            Elastos::Core::Math::Min((Int32) Elastos::Core::Math::Ceil(numItems / (Float)mMaxItemsPerRow), mMaxRows);
...
AutoPtr<IIconMenuItemView> IconMenuView::CreateMoreItemView()
{
    ...
    reinterpret_cast<IconMenuItemView*>(itemView->Probe(EIID_IconMenuItemView))->Initialize(text, mMoreIcon);
    ...
    /* [in] */ IKeyEvent* event)
{
    ...
    if (keyCode == IKeyEvent::KEYCODE_MENU) {
        ...
        if (action == IKeyEvent::ACTION_DOWN && repeatCount == 0) {
            RemoveCallbacks(THIS_PROBE(IRunnable));
            PostDelayed(THIS_PROBE(IRunnable), CViewConfiguration::GetLongPressTimeout());
        }
...
ECode IconMenuView::Run()
{
    ...
    PostDelayed(THIS_PROBE(IRunnable), ITEM_CAPTION_CYCLE_DELAY);
    ...
    /* [in] */ Int32 width)
{
    ...
    for (Int32 i = 0; i < numItems; i++) {
        AutoPtr<IIconMenuViewLayoutParams> lp;
        GetChildAt(i)->GetLayoutParams((IViewGroupLayoutParams**)&lp);
    ...
    /* [in] */ IParcelable* state)
{
    ...
    AutoPtr<IView> v = GetChildAt(focusedPosition);
    if (v != NULL) {
        Boolean rst;
        v->RequestFocus(&rst);
    }
