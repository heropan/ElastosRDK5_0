...
public:
    ...
    inline const Float* getAdvances() const { return mAdvances.array(); }
    inline size_t getAdvancesCount() const { return mAdvances.size(); }
    inline Float getTotalAdvance() const { return mTotalAdvance; }
    inline const Char16* getGlyphs() const { return mGlyphs.array(); }
    inline size_t getGlyphsCount() const { return mGlyphs.size(); }
    inline const Float* getPos() const { return mPos.array(); }
    inline size_t getPosCount() const { return mPos.size(); }

    /**
     * Advances vector
     */
    Vector<Float> mAdvances;
...
public:
    ...
    sp<TextLayoutValue> getValue(const SkPaint* paint, const Char16* text, Int32 start,
