...
public:
    ...
    class InnerVSyncRunnable : Runnable
    {
    public:
        InnerVSyncRunnable(
            /* [in] */ VSyncMonitor* owner);
    ...
    class SyntheticVSyncRunnable : Runnable
    {
    public:
...
private:
    ...
    AutoPtr<IChoreographer::IFrameCallback> mVSyncFrameCallback;

    // On ICS we just post a task through the handler (http://crbug.com/156397)
    AutoPtr<Runnable> mVSyncRunnableCallback;
    ...
    AutoPtr<Runnable> mSyntheticVSyncRunnable;
...
#endif // _ELASTOS_DROID_WEBKIT_UI_VSYNCMONITOR_H_
