...
#include "droid/test/suitebuilder/CTestSuiteBuilder.h"
    ...
    /* [in] */ IClassInfo* clazz)
{
    ...
    StringBuf_<256> name;
    clazz->GetName(&name);
    String nameStr(name.GetPayload());
    return constructor(nameStr, NULL);
}
    ...
    /* [in] */ IClassLoader* classLoader)
{
    ...
    mTestCases = list;
    return AddRequirements(NULL/*REJECT_SUPPRESSED*/);
}
    ...
    /* [in] */ IContext* context)
{
    ...
    Boolean result;
    mTestCases->AddAll(testCases, &result);
    return NOERROR;
}
...
{
    Boolean result;
    mPredicates->AddAll(predicates, &result);
    return NOERROR;
}
    ...
    /* [out] */ ITestSuite** suite)
{
    ...
    tests->GetSize(&N);
    for (Int32 i = 0; i < N; i++) {
        AutoPtr<ITestMethod> test;
        tests->Get(i, (IInterface**)&test);
    ...
    /* [in] */ ITestMethod* test)
{
    ...
    while (it->HasNext(&hasNext), hasNext) {
        AutoPtr<IPredicate> predicate;
        it->Next((IInterface**)&predicate);
        Boolean result;
