    ...
    /* [in] */ const String& packageName)
{
    ...
    while (it->HasNext(&hasNext), hasNext) {
        ...
        EcoFile::ClassInfo* classInfo = (EcoFile::ClassInfo*)elem.Get();
        ...
        Boolean result;
        topLevelClasses->Add(classInfo, &result);
    ...
    /* [in] */ ISet* subpackageNames)
{
    ...
    while (epkClassNames->HasMoreElements(&result), result) {
        AutoPtr<IInterface> elem;
        epkClassNames->NextElement((IInterface**)&elem);
        EcoFile::ClassInfo* classInfo = (EcoFile::ClassInfo*)elem.Get();
        StringBuf_<512> _name;
        classInfo->mClassInfo->GetName(&_name);
        String className((const char*)_name);
        if (className.StartWith(packageName)) {
            ...
            if (subPackageName.GetLength() > packageName.GetLength()) {
                ...
                CStringWrapper::New(subPackageName, (ICharSequence**)&csq);
