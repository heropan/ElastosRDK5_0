    ...
    /* [in] */ const String& testMethodName)
{
    ...
    if (ShouldRunSingleTestMethod(testMethodName, testClass)) {
        ...
        testClass->GetName(&nameBuf);
        mTestClassName = (const char*)nameBuf;
    }
    else {
        AutoPtr<ITest> test;
    ...
    /* [out] */ IClassInfo** clazz)
{
    ...
    ec = moduleInfo->GetClassInfo(testClassName.string(), clazz);
    if (FAILED(ec)) {
    ...
    /* [in] */ const String& testMethodName)
{
    ...
    ECode ec = NOERROR;
    BufferOf<IConstructorInfo *>* consInfos = BufferOf<IConstructorInfo *>::Alloc(count);
    ...
    for (Int32 i = 0; i < count; i++) {
        ...
        constructor->GetParamCount(&paramCount);
        if (paramCount == 1) {
            BufferOf<IParamInfo*>* params = BufferOf<IParamInfo*>::Alloc(1);
...
FOUND:
    ...
    BufferOf<IConstructorInfo *>::Free(consInfos);
    return testCase;
}
    ...
    /* [out] */ ITestCase** testCase)
{
    ...
    if (FAILED(ec)) {
        ...
        Logger::E(TAG, "Constructor thew an exception. Class: %s", (const char*)name);
        *testCase = NULL;
        return E_RUNTIME_EXCEPTION;
    }
    ...
    /* [in] */ IClassInfo* testClass)
{
    ...
    testClass->GetInterfaceInfo("ITestCase", (IInterfaceInfo**)&itfInfo);
    ...
    /* [out] */ ITest** test)
{
    ECode ec = NOERROR;
    AutoPtr<IInterfaceInfo> itfInfo;
    clazz->GetInterfaceInfo("ITestSuiteProvider", (IInterfaceInfo**)&itfInfo);
    if (itfInfo != NULL) {
        AutoPtr<IInterface> object;
        Int32 count;
        ...
        if (FAILED(ec)) {
            StringBuf_<512> name;
            clazz->GetName(&name);
            Logger::E(TAG, "Could not instantiate test suite provider. Class: %s", (const char*)name);
            return E_RUNTIME_EXCEPTION;
        }
    ...
    ec = clazz->GetMethodInfo("suite", (IMethodInfo**)&suiteMethod);
    if (FAILED(ec)) {
