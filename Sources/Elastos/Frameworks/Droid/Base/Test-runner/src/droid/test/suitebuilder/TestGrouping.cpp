...
#include "droid/test/suitebuilder/TestGrouping.h"
...
//==================================================================
CAR_INTERFACE_IMPL(TestGrouping::SortBySimpleName, IComparator)
...
//==================================================================
CAR_INTERFACE_IMPL(TestGrouping::SortByFullyQualifiedName, IComparator)
...
//==================================================================
CAR_INTERFACE_IMPL(TestGrouping::TestCasePredicate, IPredicate)
    ...
    /* [out] */ Boolean* result)
{
    ...
    aClass->GetInterfaceInfo("ITestCase", (IInterfaceInfo**)&itfInfo);
    if (itfInfo == NULL) {
        *result = FALSE;
        return NOERROR;
    }
    ...
    /* [in] */ IClassInfo* aClass)
{
    ...
    for (Int32 i = 0; i < N; i++) {
        ...
        if (params->GetUsed() == 0) return TRUE;
        if (params->GetUsed() == 1) {
    ...
    Logger::I(TAG, "TestCase class %s is missing a public constructor with no parameters "
            "or a single String parameter - skipping", (const char*)name);
    return FALSE;
...
//==================================================================
CAR_INTERFACE_IMPL(TestGrouping::TestMethodPredicate, IPredicate)
    ...
    /* [out] */ Boolean* result)
{
    ...
    StringBuf_<512> name;
    method->GetName(&name);
    *result = ((count == 0) && (CString((const char*)name).StartWith("Test")));
    return NOERROR;
...
CAR_INTERFACE_IMPL(TestGrouping, IInterface)
    ...
    /* [in] */ IClassInfo* testCaseClass)
{
    ...
    BufferOf<IMethodInfo*>* methodInfos = BufferOf<IMethodInfo*>::Alloc(N);
    ...
    /* [in] */ ArrayOf<String>* packageNames)
{
    if (packageNames != NULL) {
        for (Int32 i = 0; i < packageNames->GetLength(); i++) {
            ...
            mTestCaseClasses->AddAll(addedClasses, &result);
    ...
    /* [in] */ ArrayOf<String>* packageNames)
{
    if (packageNames != NULL) {
        for (Int32 i = 0; i < packageNames->GetLength(); i++) {
            ...
            Boolean result;
            mTestCaseClasses->RemoveAll(removedClasses, &result);
        }
    }
    return this;
}
    ...
    /* [in] */ ISet* allClasses)
{
    ...
    AutoPtr<IList> classes = Select(allClasses, predicate);
    Int32 N;
    classes->GetSize(&N);
    for (Int32 i = 0; i < N; i++) {
