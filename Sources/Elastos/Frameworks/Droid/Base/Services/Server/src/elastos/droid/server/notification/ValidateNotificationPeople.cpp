    ...
    /* [in] */ IBundle* extras)
{
    ...
    IArrayOf* arrayOf = IArrayOf::Probe(people);
    if (arrayOf != NULL) {
        InterfaceID typeId;
        arrayOf->GetTypeId(&typeId);
        if (typeId == EIID_IString) {
            Int32 size;
        ...
        else if (typeId == EIID_IChar32) {
            array = ArrayOf<String>::Alloc(1);
            Int32 size;
            arrayOf->GetLength(&size);
            AutoPtr< ArrayOf<Char32> > cArray = ArrayOf<Char32>::Alloc(size);
            for (Int32 i = 0; i < size; ++i) {
                AutoPtr<IInterface> obj;
                arrayOf->Get(i, (IInterface**)&obj);
                IChar32::Probe(obj)->GetValue(&(*cArray)[i]);
            }
            (*array)[0] = *(new String(*cArray));
            return array;
        }
        else if (typeId == EIID_ICharSequence) {
            Int32 N;
            arrayOf->GetLength(&N);
            ...
            for (Int32 i = 0; i < N; ++i) {
                AutoPtr<IInterface> obj;
                arrayOf->Get(i, (IInterface**)&obj);
                ICharSequence::Probe(obj)->ToString(&(*array)[i]);
            }
    ...
    if (IArrayList::Probe(people) != NULL) {
        AutoPtr<IArrayList> arrayList = IArrayList::Probe(people);

        Boolean res;
        if (arrayList->IsEmpty(&res), res) {
            return NULL;
        }

        AutoPtr<IInterface> obj;
        arrayList->Get(0, (IInterface**)&obj);
        ...
        if (ICharSequence::Probe(obj) != NULL) {
            AutoPtr<IArrayList> charSeqList = arrayList;
            Int32 N;
            charSeqList->GetSize(&N);
            array = ArrayOf<String>::Alloc(N);
            for (Int32 i = 0; i < N; ++i) {
                AutoPtr<IInterface> object;
                charSeqList->Get(i, (IInterface**)&object);
                ICharSequence::Probe(object)->ToString(&(*array)[i]);
            }
            return array;
        }
    ...
    if (IString::Probe(people) != NULL) {
        array = ArrayOf<String>::Alloc(1);
        array->Set(0, Object::ToString(people));
        return array;
    }

    if (ICharSequence::Probe(people) != NULL) {
        array = ArrayOf<String>::Alloc(1);
        ICharSequence::Probe(people)->ToString(&(*array)[0]);
        return array;
    }
