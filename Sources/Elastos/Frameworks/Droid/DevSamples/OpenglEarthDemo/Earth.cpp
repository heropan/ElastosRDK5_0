    ...
    /* [in] */ Float r)
{
    ...
    for(Int32 i = 0; it != alVertix.End(); i++, it++){
        (*vertices)[i] = *it;
    }
    //创建顶点坐标数据缓冲
...
ECode Earth::InitShader(
    /* [in] */ IGLSurfaceView* mv)
{
    AutoPtr<IResources> res;
    mv->GetResources((IResources**)&res);
    //加载顶点着色器的脚本内容
    mVertexShader = ShaderUtil::LoadFromAssetsFile(String("vertex_earth.sh"), res);
    //加载片元着色器的脚本内容
    mFragmentShader = ShaderUtil::LoadFromAssetsFile(String("frag_earth.sh"), res);
    //基于顶点着色器与片元着色器创建程序
    ...
    gl->GlGetUniformLocation(mProgram, String("sTextureDay"), &mDayTexHandle);
    gl->GlGetUniformLocation(mProgram, String("sTextureNight"), &mNightTexHandle);
    //获取位置、旋转变换矩阵引用id
    gl->GlGetUniformLocation(mProgram, String("uMMatrix"), &muMMatrixHandle);
    return NOERROR;
}

ECode Earth::DrawSelf(
    /* [in] */ Int32 texId,
    /* [in] */ Int32 texIdNight)
{
    AutoPtr<IGLES20> gl;
    ...
    gl->GlActiveTexture(IGLES20::_GL_TEXTURE1);
    gl->GlBindTexture(IGLES20::_GL_TEXTURE_2D, texIdNight);
    gl->GlUniform1i(mDayTexHandle, 0);
    gl->GlUniform1i(mNightTexHandle, 1);
    //绘制三角形
    gl->GlDrawArrays(IGLES20::_GL_TRIANGLES, 0, mVCount);
    return NOERROR;
}

AutoPtr<ArrayOf<Float> > Earth::GenerateTexCoor(
    /* [in] */ Int32 bw,
    /* [in] */ Int32 bh)
{
