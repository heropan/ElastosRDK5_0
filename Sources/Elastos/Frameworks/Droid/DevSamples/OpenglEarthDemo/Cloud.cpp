...
ECode Cloud::InitShader(
    /* [in] */ IGLSurfaceView* mv)
{
    AutoPtr<IResources> res;
    mv->GetResources((IResources**)&res);
    //加载顶点着色器的脚本内容
    mVertexShader = ShaderUtil::LoadFromAssetsFile(String("vertex_cloud.sh"), res);
    //加载片元着色器的脚本内容
    mFragmentShader = ShaderUtil::LoadFromAssetsFile(String("frag_cloud.sh"), res);
    //基于顶点着色器与片元着色器创建程序
...
AutoPtr<ArrayOf<Float> > Cloud::GenerateTexCoor(
    /* [in] */ Int32 bw,
    /* [in] */ Int32 bh)
{
