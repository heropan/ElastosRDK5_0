...
AutoPtr<IDate> InitLastLevelChange()
{
    AutoPtr<IDate> date;
            ...
            if (changeTime != 0) {
                ...
                if (sCharging) {
                    if (dLevel < 0) {
                        ...
                        sRemainingTime = Elastos::Core::Math::Round(dt / dLevel * (1.0 - sLevel));
                    }
                }
                else {
                    if (dLevel > 0) {
                        ...
                        sRemainingTime = Elastos::Core::Math::Round(dt / -dLevel * sLevel);
                     }
                }
...
ArrayOf<Double>* CGeckoBatteryManager::GetCurrentInformation()
{
    ArrayOf<Double>* info = ArrayOf<Double>::Alloc(3);
    (*info)[0] = GetLevel();
