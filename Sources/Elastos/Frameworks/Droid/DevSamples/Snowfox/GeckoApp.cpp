...
//        AutoPtr<IApplication> app;
//        FAIL_RETURN(host->GetApplication((IApplication**)&app));
//        String path("/data/data/com.elastos.runtime/elastos/Snowfox");
...
//        FAIL_RETURN(GeckoAppShell::LoadGeckoLibs(path));
        ...
        FAIL_RETURN(((GeckoApp*)mHost)->GetBaseContext((IContext**)&pIContext));
...
const String GeckoApp::TAG("GeckoApp");
...
{
    DBG_LOG("\t + GeckoApp::ShowErrorDialog()");
    ...
    /* [in] */ void *arg)
{
    ...
    DBG_LOG("\t + GeckoApp::LaunchEntryRoutine()");
#if (DBG)
    if (arg == NULL) {
        Logger::E(DBG_TAG, "\t \t LaunchEntryRoutine's param is NULL");
        return NOERROR;
    }
#endif
...
//    Int64 startup_time = System::GetCurrentTimeMillis();
    ...
    if (host->mLibLoadThread != NULL) {
        //host->mLibLoadThread->Join();
        Logger::W(DBG_TAG, "\t \t Then libLoadThread is running");
    }
    String action;
    closure->intent->GetAction(&action);
    if (action.Equals("android.intent.action.VIEW")) {//action.Equals(Intent_ACTION_VIEW)) {
        DBG_LOG("\t \t Intent's action is View");
        String dataString;
        closure->intent->GetDataString(&dataString);
        GeckoApp::sSurfaceView->sSplashURL = dataString;
    }

    DBG_LOG("\t \t Call DrawSpashScreen");
    ASSERT_SUCCEEDED(GeckoApp::sSurfaceView->DrawSplashScreen());

    // unpack files in the components directory
    //try {
    ECode ec = host->UnpackComponents();
    if (ec == (ECode)E_FILE_NOT_FOUND_EXCEPTION) {
        Logger::E(DBG_TAG, "\t\t error unpacking components: file not found");
        //Looper:Prepare();
        String error;
        host->GetString(0x7f050004, &error); //R.string.error_loading_file
        host->ShowErrorDialog(error);
        ...
        Logger::E(DBG_TAG, "\t\t error unpacking components: io error");
        /*String msg("io error");
        //Looper::Prepare();
        if (msg != null && msg.equalsIgnoreCase("No space left on device"))
            showErrorDialog(getString(0x7f050003)); //R.string.no_space_to_start_error
        ...
        Logger::E(DBG_TAG, "\t\t unpacking components by: %x", ec);
        return (void*)ec;
    }
    /*} catch (FileNotFoundException fnfe) {
        Log.e("GeckoApp", "error unpacking components", fnfe);
...
Boolean GeckoApp::Launch(
    /* [in] */ IIntent* _intent)
{
    DBG_LOG("\t + GeckoApp::Launch()");
    if (!CheckAndSetLaunchState(LaunchState_Launching, LaunchState_Launched)) {
        Logger::E(DBG_TAG, "Error: Set status from launching to launched fail");
        return FALSE;
    }
    ...
    if (pthread_create(&thread, NULL, LaunchEntryRoutine, (void*)closure)) {
        Logger::E(DBG_TAG, "Error: Create launch entry routine thread fail");
        return FALSE;
    }
    ...
    /* [in] */ IBundle* savedInstanceState)
{
    ...
    if (!sGREDir) {
        String dataDir("/data/data/com.elastos.runtime/elastos/Snowfox");
    ...
    Logger::D(GeckoApp::TAG, grePath.string());
    #endif
    ...
    #if (DBG)
    Logger::D(GeckoApp::TAG, "\t sFullScreen is %d", sFullscreen);
    #endif
...
//    ASSERT_SUCCEEDED(CGeckoConnectivityReceiver::New((IBroadcastReceiver**)&mConnectivityReceiver));
    ...
    */
    if (!GeckoApp::CheckAndSetLaunchState(LaunchState_PreLaunch, LaunchState_Launching)) {
        Logger::E(DBG_TAG, "\t OnCreate return by launchState error");
        return NOERROR;
    }
...
ECode GeckoApp::EnableCameraView()
{
    ...
    DBG_LOG("\t + GeckoApp::EnableCameraView()");
...
{
    DBG_LOG("\t GeckoApp::DisableCameraView()");
...
ECode GeckoApp::OnStart()
{
    ...
    DBG_LOG("\t + GeckoApp::OnStart()");
    if (!this->CheckLaunchState(LaunchState_GeckoRunning)) {
...
{
    DBG_LOG("GeckoApp::OnConfigurationChanged");
    // nothing, just ignore
...
{
    DBG_LOG("\t + GeckoApp::UnpackComponents()");
    String resPath("/data/data/com.elastos.runtime/elastos/Snowfox/Snowfox.apk");
...
//    FAIL_RETURN(pIApplication->GetPackageResourcePath(&resPath));
    ...
    if (FAILED(ec)) {
        Logger::E(DBG_TAG, "\t GeckoApp error CZipFile::New %08x\n", ec);
        return ec;
    }
    ...
    if (FAILED(ec)) {
        Logger::E(DBG_TAG, "GeckoApp error UnpackFile removing files %08x\n", ec);
        return ec;
    }
    if (succeeded) {
        //TODO ec = RemoveFiles();
        if (FAILED(ec)) {
            ...
            Logger::E(DBG_TAG, "GeckoApp error removing files %08x\n", ec);
            return ec;
        }
    }
    ...
    while (SUCCEEDED(ec) && succeeded) {
        ...
        pEntry->GetName(&name);
        #if (DBG)
        Logger::D(DBG_TAG, "\t \t entry name: %s", name.string());
        #endif
...
ECode GeckoApp::UnpackFile(
    ...
    /* [out] */ Boolean* pResult)
{
    DBG_LOG("\t + GeckoApp::UnpackFile()");
    if (!pZip || !pResult) {
        Logger::E(DBG_TAG, "\t \t UnpackFile parameter is invalid");
...
{
    DBG_LOG("GeckoApp::CheckAndLaunchUpdate()");
    assert(0);
    ECode ec;
...
//        pIFile->GetPath(&path);
    ...
    //try {
    {
        ArrayOf<Byte> buf((Byte*)status.string(), status.GetByteLength());
