    ...
    /* [in] */ Int64 when)
{
    ...
    ec = CNotification::New(aIcon,
        (!aText.IsNullOrEmpty()) ? aTextSeq : aTitleSeq, when, (INotification**)&mNotification);
    if (FAILED(ec)) return ec;
    ...
    /* [in] */ Int32 aProgressMax)
{
    ...
    if (!mProgressStyle) {
        // Custom view
        Int32 layout = !aAlertText.IsNullOrEmpty() ?
...
//        if (FAILED(ec)) return ec;
    ...
    if (mPrevAlertText.Equals(text)
        && Elastos::Core::Math::Abs(mPrevPercent - percent) < UPDATE_THRESHOLD) {
        return NOERROR;
    }
...
ECode CAlertNotification::ToString(
    /* [out] */ String* str)
{
    return mNotification->ToString(str);
}
