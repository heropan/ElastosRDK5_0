...
{
    DBG_LOG("\t + GeckoAppShell::IMEStateUpdater::Run()");
    mClassLock.Lock();
    instance.Clear();
    mClassLock.Unlock();
...
ECode GeckoAppShell::IMEStateUpdater::ScheduledExecutionTime(
    /* [out] */ Int64* value)
{
    DBG_LOG("\t + GeckoAppShell::ScheduleExceutionTime()");
    VALIDATE_NOT_NULL(value);
...
{
    DBG_LOG("\t + GeckoAppShell::IsScheduled()");
...
{
    DBG_LOG("\t + GeckoAppShell::IsCancelled()");
...
ECode GeckoAppShell::IMEStateUpdater::GetPeriod(
    /* [out] */ Int64* period)
{
    DBG_LOG("\t + GeckoAppShell::GetPeriod()");
...
ECode GeckoAppShell::IMEStateUpdater::SetPeriod(
    /* [in] */ Int64 period)
{
    DBG_LOG("\t + GeckoAppShell::SetPeriod()");
...
ECode GeckoAppShell::IMEStateUpdater::SetFixedRate(
    /* [in] */ Boolean fixed)
{
    DBG_LOG("\t + GeckoAppShell::SetFixed");
...
{
    DBG_LOG("\t + GeckoAppShell::GetInstance");
    if (instance == NULL) {
...
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::GetmainHandler");
    DBG_LOG("\t + GeckoAppShell::GetMainHandler");
...
ECode GeckoAppShell::GetHandler(
    /* [out] */ IHandler** ppHandler)
{
    DBG_LOG("\t + GeckoAppShell::GetHandler");
    //PRINT_ENTER_LEAVE("GeckoAppShel::GetHandler");
    if (!ppHandler) return E_INVALID_ARGUMENT;
    ...
    if (!sHandler) {
        ...
        CHandler::New(mainLooper, (IHandler**)&sHandler);
...
ECode GeckoAppShell::GetCacheDir(
    /* [out] */ IFile** ppFile)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::GetCacheDir");
    DBG_LOG("\t + GeckoAppShell::GetCacheDir()");
...
{
    DBG_LOG("\t + GeckoAppShell::GetFreeSpace()");
    //try {
        if (sFreeSpace == -1) {
...
ECode GeckoAppShell::MoveFile(
    ...
    /* [out] */ Boolean* pResult)
{
    DBG_LOG("\t + GeckoAppShell::MoveFile()");
...
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::LoadGeckoLibs");
    DBG_LOG("\t + GeckoAppShell::LoadGeckoLibs");
    AutoPtr<GeckoApp> geckoApp = GeckoApp::sAppContext;
    String homeDir("/data/data/com.elastos.runtime/elastos/Snowfox");
    String resPath("/data/data/com.elastos.runtime/elastos/Snowfox/Snowfox.apk");
...
//    FAIL_RETURN(pIApplication->GetPackageResourcePath(&resPath));
    ...
    if (Build::VERSION::SDK_INT < 8 || resPath.StartWith("/data") || resPath.StartWith("/system")) {
        ...
        String oldHomePath("/data/data/com.elastos.runtime/elastos/Snowfox");
        String packageName;
...
ECode GeckoAppShell::RunGecko(
    ...
    /* [in] */ const String& url)
{
    DBG_LOG("\t + GeckoAppShell::RunGecko");
#ifdef IS_DBG
    /*
    PRINT_ENTER_LEAVE("GeckoAppShell::RunGecko");
    // apkPath: /data/data/com.elastos.runtime/elastos/Snowfox/Snowfox.apk
    if (!apkPath.IsNull())
        printf("\t apkPath: %s\n", apkPath.string());
    if (args.IsNull())
        printf("\t args: %s\n", args.string());
...
ECode GeckoAppShell::SendEventToGecko(
    /* [in] */ GeckoEvent* event)
{
    DBG_LOG("\t + GeckoAppShell::SendEventToGecko");
    // PRINT_ENTER_LEAVE("GeckoAppShell::SendEventToGecko");
    //printf("\t GeckoEvent type: %d\n", event->mType);
    if (GeckoApp::CheckLaunchState(GeckoApp::LaunchState_GeckoRunning)) {
        //printf("\t Call NotifyGeckoOfEvent()\n");
        NotifyGeckoOfEvent(event);
    }
    else {
        //printf("\t PushBackEvent\n");
...
ECode GeckoAppShell::SendEventToGeckoSync(
    /* [in] */ GeckoEvent* event)
{
    DBG_LOG("\t + GeckoAppShell::SendETSync");
    // PRINT_ENTER_LEAVE("GeckoAppShell::SendEventToGeckoSync");
...
{
    DBG_LOG("\t + GeckoAppShell::ScheuleRD");
    // Redraw everything
    //PRINT_ENTER_LEAVE("GeckoAppShell::ScheduleRedrow");
...
ECode GeckoAppShell::ScheduleRedraw(
    ...
    /* [in] */ Int32 h)
{
    DBG_LOG("\t + GeckoAppShell::SRD2");
    //PRINT_ENTER_LEAVE("GeckoAppShell::ScheduleRedraw");
...
ECode GeckoAppShell::NotifyIME(
    ...
    /* [in] */ Int32 state)
{
    // PRINT_ENTER_LEAVE("GeckoAppShell::NotifyIME");
    DBG_LOG("\t + GeckoAppShell::NotifyIME()");
    ...
    {
        DBG_LOG("\t \t NOTIFY_IME_RESETINPUTSTATE");
    ...
    case NOTIFY_IME_CANCELCOMPOSITION:
        DBG_LOG("\t \t NOTIFY_IME_CANCELCOMPOSITION");
        IMEStateUpdater::ResetIME();
        break;

    case NOTIFY_IME_FOCUSCHANGE:
        DBG_LOG("\t \t NOTIFY_IME_FOCUSCHANGE");
        IMEStateUpdater::ResetIME();
        break;
    }
...
{
    DBG_LOG("\t + GeckoAppShell::NIMEEnabled");
    // PRINT_ENTER_LEAVE("GeckoAppShell::NofifyIMEEnabled");
    if (!GeckoApp::sSurfaceView) {
        return NOERROR;
    }
...
ECode GeckoAppShell::NotifyIMEChange(
    ...
    /* [in] */ Int32 newEnd)
{
    DBG_LOG("\t + GeckoAppShell::NotifyIMEChange");
    // PRINT_ENTER_LEAVE("GeckoAppShell::NofifyIMEChange");
    if (!GeckoApp::sSurfaceView ||
...
ECode GeckoAppShell::NotifyScreenShot(
    ...
    /* [in] */ Int32 height)
{
    DBG_LOG("\t + GeckoAppShell::NotifySS");
    return NOERROR;
}
...
{
    DBG_LOG("\t + GeckoAppShell::AcknowledgeEventSync()");
    // PRINT_ENTER_LEAVE("GeckoAppShell::AcknowledgeEventSync");
    assert(0);
...
ECode GeckoAppShell::EnableDeviceMotion(
    /* [in] */ Boolean enable)
{
    DBG_LOG("\t + GeckoAppShell::EnableDeviceMotion()");
    //PRINT_ENTER_LEAVE("GeckoAppShell::EnableDeviceMotion");
...
ECode GeckoAppShell::EnableLocation(
    /* [in] */ Boolean enable)
{
    DBG_LOG("\t + GeckoAppShell::EnableLocation");
...
{
    DBG_LOG("\t + GeckoAppShell::EnableSensor");
...
ECode GeckoAppShell::DisableSensor(
    /* [in] */ Int32 aSensortype)
{
    DBG_LOG("\t + GeckoAppShell::DisableSensor");
    //AutoPtr<ISensorManager> sm;
    AutoPtr<IContext> context;
    ASSERT_SUCCEEDED(GeckoApp::sSurfaceView->GetContext((IContext**)&context));
    //ASSERT_SUCCEEDED(context->GetSystemService(Context_SENSOR_SERVICE,(ISensorManager**)sm));
...
{
    DBG_LOG("\t + GeckoAppShell::MoveTaskToBack");
    // PRINT_ENTER_LEAVE("GeckoAppShell::MoveTaskToBack");
    //Boolean result;
...
{
    DBG_LOG("\t + GeckoAppShell::ReturnIMEQueryResult");
    // PRINT_ENTER_LEAVE("GeckoAppShell::ReturnIMEQueryResult");
...
{
    DBG_LOG("\t + GeckoAppShell::OnAppShellReady");
    // PRINT_ENTER_LEAVE("GeckoAppShell::OnAppShellReady");
...
{
    DBG_LOG("\t + GeckoAppShell::OnXreExit");
    // mLaunchState can only be Launched or GeckoRunning at this point
    // PRINT_ENTER_LEAVE("GeckoAppShell::OnXreExit");
...
{
    // PRINT_ENTER_LEAVE("GeckoAppShell::ScheduleRestart");
...
{
    DBG_LOG("\t + GeckoAppShell::CreateShortcut");
    // PRINT_ENTER_LEAVE("GeckoAppShell::CreateShortcut");
...
{
    DBG_LOG("GeckoAppShell::GetHanlderForMimeType");
...
ECode GeckoAppShell::GetHandlersForURL(
    ...
    /* [out, callee] */ ArrayOf<String>** ppHandlers)
{
    DBG_LOG("GeckoAppShell::GethandlersForURL");
    if (!ppHandlers) return E_INVALID_ARGUMENT;
...
ECode GeckoAppShell::GetHandlersForIntent(
    ...
    /* [out, callee] */ ArrayOf<String>** ppHandlers)
{
    DBG_LOG("GeckoAppShell::GetHandlersForIntent");
    if (!ppHandlers) return E_INVALID_ARGUMENT;
...
{
    DBG_LOG("\t + GeckoAppShell::GetIntentForActionsString");
    if (!ppIntent) return E_INVALID_ARGUMENT;

    #if (DBG)
    Logger::D(DBG_TAG, "\t \t action: %s", aAction.string());
    #endif
...
ECode GeckoAppShell::GetExtensionFromMimeType(
    ...
    /* [out] */ String* ext)
{
    DBG_LOG("GeckoAppShell::GetExtensionFromMimeType");
    ...
    ECode ec = mimeTypeHelper->GetSingleton((IMimeTypeMap**)&mimeType);
    if (FAILED(ec)) return ec;
...
ECode GeckoAppShell::GetMimeTypeFromExtensions(
    ...
    /* [out] */ String* mimeType)
{
    DBG_LOG("\t + GeckoAppShell::GetMimeTypeFromExtensions");
    if (!mimeType) return E_INVALID_ARGUMENT;
    #if (DBG)
    Logger::D(DBG_TAG, "\t \t fileExt: %s, mimeType: %s", aFileExt.string(), mimeType->string());
    #endif
    AutoPtr<IMimeTypeMapHelper> mimeTypeHelper;
    CMimeTypeMapHelper::AcquireSingleton((IMimeTypeMapHelper**)&mimeTypeHelper);
    ...
    while (st->HasMoreTokens()) {
        String ext = String(st->NextToken());
        String mt;
...
ECode GeckoAppShell::OpenUriExternal(
    ...
    /* [in] */ const String& aTitle)
{
    DBG_LOG("\t + GeckoAppShell::OpenUriExternal");
    if (!aUriSpec.IsNullOrEmpty()) {
    if (aUriSpec.EndWithIgnoreCase("apk")) {
        String srcPath = aUriSpec.Substring(7);
        String apkName = aUriSpec.Substring(63);
        #if (DBG)
        Logger::D(DBG_TAG, "\t \t APK name: %s", apkName.string());
        #endif
        String targetPath("/data/app/");
        targetPath += apkName;
        #if (DBG)
        Logger::D(DBG_TAG, "\t \t targetPath: %s", targetPath.string());
        #endif

        GeckoAppShell::CopyFile(srcPath, targetPath);
            /*
        AutoPtr<IContext> ctxPtr;
            ECode ec = GeckoApp::sSurfaceView->GetContext((IContext**)&ctxPtr);
            if (FAILED(ec)) {
        DBG_LOG("\t \t Get Context fail");
        return ec;
        }

        AutoPtr<IPackageManager> packageManagerPtr;
            ec = ctxPtr->GetPackageManager((IPackageManager**)&packageManagerPtr);
            if (FAILED(ec)) {
        DBG_LOG("\t \t Get packageManager fail");
        return ec;
        }

        DBG_LOG("\t \t + Call InstallPackage()");
        packageManagerPtr->InstallPackage(NULL, NULL, 0, targetPath);
        DBG_LOG("\t \t - Call InstallPackage()");
        */

        DBG_LOG("\t - GeckoAppShell::OpenUriExtenal");
        return NOERROR;
    }
    }
    ...
    ECode ec = GetIntentForActionString(aAction, (IIntent**)&pIIntent);
    if (FAILED(ec)) {
    DBG_LOG("\t \t GetIntentForActionString fail");
    return ec;
    }
        ...
        if (scheme.Equals("sms")) {
            ...
            if (!query.IsNullOrEmpty()) {
                ...
                while (fields.HasMoreTokens()) {
                    String field = String(fields.NextToken());
                    if (field.GetLength() > 5 && field.Substring(0, 5).Equals("body=")) {
                        ...
                        String temp("&");
                        temp.Append(field);
                        resultQuery.Append(!resultQuery.IsNullOrEmpty() ? temp : field);
                    }
                }
                if (foundBody) {
...
ECode GeckoAppShell::GetClipboardText(
    /* [out] */ String* text)
{
    DBG_LOG("+ GeckoAppShell::GetClipboardText()");
...
ECode GeckoAppShell::SetClipboardText(
    /* [in] */ const String& text)
{
    DBG_LOG("GeckoAppShell::SetClipboardText");
    AutoPtr<IHandler> handler;
    ECode ec = GetHandler((IHandler**)&handler);
    if (FAILED(ec)) return ec;
...
{
    DBG_LOG("\t + GeckoAppShell::ShowAlertNotification");
#if (DBG)
    Logger::D(DBG_TAG, " GeckoAppShell.showAlertNotification - image = '"
            "%s'\n- title = '%s'\n- text = '%s'\n- cookie = '%s'\n- name = '%s'",
...
//    if (FAILED(ec)) return ec;
    ...
    Int64 timeMillis = Elastos::Core::System::GetCurrentTimeMillis();
...
ECode GeckoAppShell::AlertsProgressListener_OnProgress(
    ...
    /* [in] */ const String& aAlertText)
{
    DBG_LOG("GeckoAppShell::AlertsProgressListener_OnProgress");
...
ECode GeckoAppShell::AlertsProgressListener_OnCancel(
    /* [in] */ const String& aAlertName)
{
    DBG_LOG("GeckoAppShell::AlertsProgressListener_OnCancel");
    #if (DBG)
    Logger::D(DBG_TAG, " GeckoAppShell.alertsProgressListener_OnCancel('%s')\n", aAlertName.string());
    #endif
...
{
    DBG_LOG("GeckoAppShell::RemoveNotification");
    return NOERROR;                                         // Liwin temp
...
{
    DBG_LOG("GeckoAppShell::SetFullScreen");
...
ECode GeckoAppShell::ShowFilePicker(
    ...
    /* [out] */ String* pPicker)
{
    DBG_LOG("GeckoAppShell::ShowFilePicker");
...
{
    DBG_LOG("GeckoAppShell::PerformHapticFeedback");
    Boolean result;
...
{
    // PRINT_ENTER_LEAVE("GeckoAppShell::Vibrator");
    AutoPtr<IContext> context;
    ASSERT_SUCCEEDED(GeckoApp::sSurfaceView->GetContext((IContext**)&context));
...
{
    // PRINT_ENTER_LEAVE("GeckoAppShell::CanelVibrate");
    AutoPtr<IVibrator> vibrator;
    ASSERT_SUCCEEDED(Vibrator((IVibrator**)&vibrator));
...
{
    DBG_LOG("GeckoAppShell::ShowInputMethodPicker");
    AutoPtr<IContext> pIContext;
    ECode ec = GeckoApp::sSurfaceView->GetContext((IContext**)&pIContext);
    if (FAILED(ec)) return ec;
...
{
    DBG_LOG("GeckoAppShell::HideProgressDialog");
...
ECode GeckoAppShell::SetKeepScreenOn(
    /* [in] */ Boolean on)
{
    DBG_LOG("GeckoAppShell::SetKeepScreenOn");
...
{
    DBG_LOG("GeckoAppShell::IsNetWrokLink");
    /*
    ...
    *pIsLinkUp = TRUE;
    ...
    if (FAILED(ec)) {
        printf("\t Get connectivity service fail\n");
        return NOERROR;
    }

    pCm->AddRef();
    printf("\t Get Connectivity service pass\n");

    AutoPtr<INetworkInfo> pINetworkInfo;
    ec = pCm->GetActiveNetworkInfo((INetworkInfo**)&pINetworkInfo);
    pINetworkInfo->AddRef();
    if (FAILED(ec)) {
        printf("\t GetNetworkInfo fail\n");
        return NOERROR;
    }
    printf("\t Get NetworkInfo interface pass\n");

    if (!pINetworkInfo) {
         printf("\t INetworkInfo is NULL \n");
         *pIsLinkUp = FALSE;
         return NOERROR;
    }
    ...
    if (FAILED(ec)) {
        printf("\t IsConnected fail \n");
        return NOERROR;
    }
    printf("\t Call IsConnected pass\n");

    if (!isConnected) {
         *pIsLinkUp = FALSE;
         printf("\t  Note: ***********************Network is offline *******************\n\n");
     }
    else {
         *pIsLinkUp = TRUE;
     }
...
{
    DBG_LOG("GeckoAppShell::IsNetworkLinkKnown");
...
{
    DBG_LOG("\t  + GeckoAppShell::SetSelectedLocale");
...
{
    DBG_LOG("GeckoAppShell::GetSystemColors");
...
{
    DBG_LOG("GeckoAppShell::PutchildInBackground");
...
{
    DBG_LOG("GeckoAppShell::PutChildInForeground");
...
{
    DBG_LOG("GeckoAppShell::KillAnyZombies");
...
ECode GeckoAppShell::CheckForGeckoProcs(
    /* [out] */ Boolean* pResult)
{
    DBG_LOG("GeckoAppShell::CheckForGeckoProcs");
    if (!pResult) return E_INVALID_ARGUMENT;
...
{
    DBG_LOG("GeckoAppShell::WaitForAnotherGeckoProc");
    Int32 countDown = 40;
    Boolean result;
...
ECode GeckoAppShell::ScanMedia(
    ...
    /* [in] */ const String& aMimeType)
{
    DBG_LOG("GeckoAppShell::ScanMedia");
    #if (DBG)
    Logger::D(DBG_TAG, "\t \t file: %s, type: %s", aFile.string(), aMimeType.string());
    #endif
...
{
    DBG_LOG("GeckoAppShell::GetIConForExtension");
    //try {
...
Void GeckoAppShell::NotifyGeckoOfEvent(
    /* [in] */ GeckoEvent* event)
{
    DBG_LOG("\t + GeckoAppShell::NotifyGeckoOfEvent");
...
{
    DBG_LOG("GeckoAppShell::NativeInit");
    InitEGEnv();

    // The package data lib directory isn't placed in ld.so's
    // search path, so we have to manually load libraries that
    // libxul will depend on.  Not ideal.
    void* handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libmozglue.so", RTLD_LAZY);
    ...
    String apkName("/data/data/com.elastos.runtime/elastos/Snowfox/Snowfox.apk");

    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libmozalloc.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }

    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libmozsqlite3.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }

    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libnspr4.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libplc4.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libplds4.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libnssutil3.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libnss3.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libsmime3.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libssl3.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libxul.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    assert(handle != NULL);
    ...
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libxpcom.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libnssckbi.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libfreebl3.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }
    handle = dlopen("/data/data/com.elastos.runtime/elastos/Snowfox/libsoftokn3.so", RTLD_LAZY);
    if (handle == NULL) {
        printf("failed: %s\n", dlerror());
    }

    g_fNativeInit(gEGEnv, NULL);
    String home("HOME=/data/data/com.elastos.runtime/elastos/Snowfox");
    // home.Append(homeDir);
    String greHome("GRE_HOME=/data/data/com.elastos.runtime/elastos/Snowfox");
    // String grePath;
    // GeckoApp::sGREDir->GetPath(&grePath);
    // greHome.Append(grePath);
    GeckoAppShell::PutEnv(home);
    ...
    //pIFile->GetPath(&tmpPath);
    //pIFile->Release();
    String extStorage("EXTERNAL_STORAGE=/data/data/com.elastos.runtime/elastos/Snowfox/tmp");
    // extStorage.Append(tmpPath);
    GeckoAppShell::PutEnv(extStorage);


    String cachePath("CACHE_PATH=/data/data/com.elastos.runtime/elastos/Snowfox/cache");
    // cachePath.Append(tmpPath);
    GeckoAppShell::PutEnv(cachePath);

    String downloadDir("DOWNLOADS_DIRECTORY=/data/data/com.elastos.runtime/elastos/Snowfox/download");
    // downloadDir.Append(tmpPath);
    GeckoAppShell::PutEnv(downloadDir);

    String updateDir("UPDATES_DIRECTORY=/data/data/com.elastos.runtime/elastos/Snowfox/update");
    // updateDir.Append(tmpPath);
    GeckoAppShell::PutEnv(updateDir);
...
Void GeckoAppShell::NativeRun(
    /* [in] */ const String& args)
{
    DBG_LOG("GeckoAppShell::NativeRun");
...
{
    DBG_LOG("GeckoAppShell::SetSurfaceView");
...
{
    DBG_LOG("GeckoAppShell::OnResume");
    return g_fOnResume(gEGEnv);
}

Void GeckoAppShell::OnLowMemory()
{
    DBG_LOG("GeckoAppShell::OnLowMemory");
...
{
    DBG_LOG("GeckoAppShell::CallObserver");
...
Void GeckoAppShell::RemoveObserver(
    /* [in] */ const String& observerKey)
{
    DBG_LOG("GeckoAppShell::RemoveObserver");
    #if (DBG)
    Logger::D(DBG_TAG, "\t \t observerKey: %s", observerKey.string());
    #endif
...
Void GeckoAppShell::LoadGeckoLibsNative(
    /* [in] */ const String& apkName)
{
    DBG_LOG("GeckoAppShell::LoadGeckoLibsNative");
...
Void GeckoAppShell::LoadSQLiteLibsNative(
    /* [in] */ const String& apkName)
{
    DBG_LOG("GeckoAppShell::LoadSQLiteLibsNative");
...
Void GeckoAppShell::LoadNSSLibsNative(
    /* [in] */ const String& apkName)
{
    DBG_LOG("GeckoAppShell::LoadNSSLibsNative");
...
{
    DBG_LOG("GeckoAppShell::OnChangeNetworkLinStatus");
...
{
    DBG_LOG("GeckoAppShell::RePortJavaCrash");
    g_fReportJavaCrash(gEGEnv, stack.string());
}

Void GeckoAppShell::ProcessNextNativeEvent()
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::ProcessNextNativeEvent");
    DBG_LOG("\t + GeckoAppShell::ProcessNextNativeEvent");
...
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifyBatteryChange");
...
Void GeckoAppShell::NotifySmsReceived(
    ...
    /* [in] */ Int32 aTimestamp)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NofifySmsReceived");
...
Int32  GeckoAppShell::SaveMessageInSentbox(
    ...
    /* [in] */ Int64 aTimestamp)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::SaveMessageInSentbox");
    DBG_LOG("\t + GeckoAppShell::SaveMsgInSentBox");
...
Void GeckoAppShell::NotifySmsSent(
    ...
    /* [in] */ Int32 aProcessId)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NofifySmsSent");
...
Void GeckoAppShell::NotifySmsDelivered(
    ...
    /* [in] */ Int32 aTimestamp)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifySmsDelivered");
...
Void GeckoAppShell::NotifySmsSendFailed(
    ...
    /* [in] */ Int64 aProcessId)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifySmsSendFailed");
...
Void GeckoAppShell::NotifyGetSms(
    ...
    /* [in] */ Int32 aProcessId)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifyGetSms");
...
Void GeckoAppShell::NotifyGetSmsFailed(
    ...
    /* [in] */ Int32 aProcessId)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifyGetSmsFailed");
    DBG_LOG("\t + GeckoAppShell::NotifyGetSmsFailed");
...
Void GeckoAppShell::NotifySmsDeleted(
        ...
        /* [in] */ Int32 aProcessId)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifySmsDeleted");
    DBG_LOG("\t + GeckoAppShell::NotifySmsDel");
...
Void GeckoAppShell::NotifySmsDeleteFailed(
    ...
    /* [in] */ Int32 aProcessId)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifySmsDeleteFailed");
    DBG_LOG("\t + GeckoAppShell::NotifySmsDelFailed");
...
Void GeckoAppShell::NotifyNoMessageInList(
    ...
    /* [in] */ Int32 aProcessId)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifyNoMessageInList");
    DBG_LOG("\t + GeckoAppShell::NotifyNoMessageInList");
...
Void GeckoAppShell::NotifyListCreated(
    ...
    /* [in] */ Int32 aProcessId)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifyListCreated");
    DBG_LOG("\t + GeckoAppShell::NotifyListCreated");
...
Void GeckoAppShell::NotifyGotNextMessage(
    ...
    /* [in] */ Int32 aProcessId)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifyGotNextMessage");
    DBG_LOG("\t + GeckoAppShell::NotifyNMessage");
...
Void GeckoAppShell::NotifyReadingMessageListFailed(
    ...
    /* [in] */ Int32 aProcessId)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::NotifyReadingMessageListFailed");
    DBG_LOG("\t + GeckoAppShell::NotifyReadingMessageListFailed");
    g_fNotifyReadingMessageListFailed(gEGEnv, aError, aRequestId, aProcessId);
}

ECode GeckoAppShell::PutLocaleEnv()
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::PutLocaleEnv");
    DBG_LOG("\t + GeckoAppShell::PutLocaleEnv");
...
{
    DBG_LOG("\t + GeckoAppShell::SendPendingEventsToGecko");
    //PRINT_ENTER_LEAVE("GeckoAppShell::SendPendingEventsToGecko");
    //try {
...
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::EnumerateGeckoProcesses");
...
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::GetDrawableForExtension");
    DBG_LOG("\t + GeckoAppShell::GetDforExt");
    AutoPtr<IIntent> pIIntent;
    ...
    CMimeTypeMapHelper::AcquireSingleton((IMimeTypeMapHelper**)&mimeTypeHelper);
    AutoPtr<IMimeTypeMap> pMtm;
    ec = mimeTypeHelper->GetSingleton((IMimeTypeMap**)&pMtm);
    if (FAILED(ec)) return ec;
...
ECode GeckoAppShell::GetShowPasswordSetting(
    /* [out] */ Boolean*  isShow)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::GetShowPasswordSetting");
    DBG_LOG("\t + GeckoAppShell::GetSPSetings");
    *isShow = TRUE;
    ECode ec;
    Int32 showPassword;
...
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::GetAccessibilityEnable");
    DBG_LOG("\t + GeckoAppShell::GetAccEnabled");
    AutoPtr<IContext> pIContext;
    ECode ec = GeckoApp::sSurfaceView->GetContext((IContext**)&pIContext);
    if (FAILED(ec)) return ec;
...
public:
    ...
    ECode Run()
    {
        DBG_LOG("\t + GeckoAppShell::AddPluginViewRunnable");
...
ECode GeckoAppShell::AddPluginView(
    ...
    /* [in] */ Double h)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::AddPluginView");
    DBG_LOG("\t + GeckoAppShell::AdddPluginView");
...
public:
    ...
    ECode Run()
    {
        DBG_LOG("\t + GeckoAppShell::RemovePluginViewRunnable");
        assert(0);
...
ECode GeckoAppShell::RemovePluginView(
    /* [in] */ IView* view)
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::RemovePluginView");
    DBG_LOG("\t + GeckoAppShell::RemovePluginView");
    AutoPtr<IHandler> handler;
    ECode ec = GetHandler((IHandler**)&handler);
    if (FAILED(ec)) return ec;
...
{
    DBG_LOG("\t + GeckoAppShell::GetSurfaceInfo");
...
{
    //PRINT_ENTER_LEAVE("GeckoAppShell::ExecuteNextRunnable");
    DBG_LOG("\t + GeckoAppShell::ExecuteNextRunnable");
...
public:
    ...
    ECode Run()
    {
        ...
        DBG_LOG("\t + GeckoAppShell::GeckoRunnableCallback");
...
{
    DBG_LOG("\t + GeckoAppShell::PostToJavaThread");
    assert(0);
...
{
    DBG_LOG("\t + GeckoAppShell::ClearMessageList");
...
AutoPtr<ArrayOf<Byte> > GeckoAppShell::sCameraBuffer;
    ...
    /* [out] */ ArrayOf<Int32>** result)
{
    ...
    *result = ArrayOf<Int32>::Alloc(4);
    ...
    while (SUCCEEDED(ec) && succeeded) {
        ...
        Int32 delta = Elastos::Core::Math::Abs(nFps - sKPreferedFps);

        if (delta < fpsDelta) {
            fpsDelta = Elastos::Core::Math::Abs(nFps - sKPreferedFps);
...
{
    DBG_LOG("\t + GeckoAppShell::ClearMessageList");
    //PRINT_ENTER_LEAVE("GeckoAppShell::HandleGeckoMessage");
...
{
    DBG_LOG("\t + GeckoAppShell::ClearMessageList");
    AutoPtr<ISmsManager> mgr = SmsManager::GetInstance();
    if (mgr == NULL) {
        *num = 0;
        return NOERROR;
    }

    return mgr->GetNumberOfMessagesForText(aText, num);
    //return NOERROR;
}
    ...
    /* [in] */ Int64 aProcessId)
{
    ...
    mgr->Send(aNumber, aMessage, aRequestId, aProcessId);
    return NOERROR;
}
    ...
    /* [in] */ Int32* errorCode)
{
    ...
    return mgr->SaveSentMessage(aRecipient, aBody, aDate, errorCode);
}
    ...
    /* [in] */ Int64 aProcessId)
{
    ...
    return mgr->GetMessage(aMessageId, aRequestId, aProcessId);
}
    ...
    /* [in] */ Int64 aProcessId)
{
    ...
    return mgr->DeleteMessage(aMessageId, aRequestId, aProcessId);
}
    ...
    /* [in] */ Int64 aProcessId)
{
    ...
    return mgr->GetNextMessageInList(aListId, aRequestId, aProcessId);
}
    ...
    /* [in] */ Int32 aListId)
{
    ...
    return mgr->ClearMessageList(aListId);
}

ECode GeckoAppShell::IsTablet(
    /* [out] */ Boolean *isTablet)
{
    DBG_LOG("\t + GeckoAppShell::IsTablet");
...
ECode GeckoAppShell::GetCurrentNetworkInformation(
    /* [out] */ ArrayOf<Double>** info)
{
    DBG_LOG("\t + GeckoAppShell::GetCurrentNetworkInformation");
...
{
    DBG_LOG("\t + GeckoAppShell::EnableNetworkNotifications");
    AutoPtr<CGeckoNetworkManager> mgr
        = CGeckoNetworkManager::GetInstance();
...
{
    DBG_LOG("\t + GeckoAppShell::DisableNetworkNotifications");
    AutoPtr<CGeckoNetworkManager> mgr
        = CGeckoNetworkManager::GetInstance();
...
{
    DBG_LOG("\t + GeckoAppShell::SetPreventPanning");
    return NOERROR;
}

void GeckoAppShell::CopyFile(const String& srcStr, const String& targetStr) {
    DBG_LOG("\t + GeckoAppShell::CopyFile()");
#if (DBG)
    assert(!srcStr.IsNullOrEmpty());
    assert(!targetStr.IsNullOrEmpty());
#endif

    FILE* srcFilePtr = NULL;
    FILE* targetFilePtr = NULL;
    char* bufPtr = (char*)malloc(1024);
    char* tmpPtr;
    Int32 readNum, writeNum;

    if (NULL == bufPtr) {
        Logger::D(DBG_TAG, "\t \t warning: malloc buffer fail");
        goto exit;
    }

    if (NULL == (srcFilePtr = fopen(srcStr.string(), "rb"))) {
        Logger::D(DBG_TAG, "\t \t warning: Open source file failed");
        goto exit;
    }

    if (NULL == (targetFilePtr = fopen(targetStr.string(), "wb+"))) {
        Logger::D(DBG_TAG, "\t \t warning: Open target file faild");
        goto exit;
    }

    while (0 != (readNum = fread(bufPtr, 1, 1024, srcFilePtr))) {
        tmpPtr = bufPtr;
        while (writeNum = fwrite(tmpPtr, 1, readNum, targetFilePtr)) {
            if ((writeNum == readNum) || (0 == writeNum))
                break;
            else {
                tmpPtr  += writeNum;
                readNum -= writeNum;
