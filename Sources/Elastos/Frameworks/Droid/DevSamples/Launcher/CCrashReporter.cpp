...
//    if (FAILED(ec)) return ec;
    ...
    ec = CFile::New(
        String("/data/data/com.elastos.runtime/elastos/Launcher/mozilla/Crash Reports/pending"),
        (IFile**)&pPendingDir);
    if (FAILED(ec)) return ec;
    ...
    ec = CFile::New(passedMinidumpPath.Replace(".dmp", ".extra"), (IFile**)&pExtrasFile);
    if (FAILED(ec)) return ec;
