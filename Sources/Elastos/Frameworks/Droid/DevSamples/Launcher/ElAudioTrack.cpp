...
const char* ElAudioTrack::TAG = "ElAudioTrack";
    ...
    /* [in] */ int audioFormat)
{
    ...
    case AudioFormat::CHANNEL_CONFIGURATION_STEREO:
        ...
        break;
    default:
        ALOGE(TAG, "getMinBufferSize(): Invalid channel configuration.");
        return ERROR_BAD_VALUE;
    }

    if ((audioFormat != AudioFormat::ENCODING_PCM_16BIT)
        && (audioFormat != AudioFormat::ENCODING_PCM_8BIT)) {
        ALOGE(TAG, "getMinBufferSize(): Invalid audio format.");
        return ERROR_BAD_VALUE;
    }

    if ( (sampleRateInHz < 4000) || (sampleRateInHz > 48000) ) {
        ALOGE(TAG, "getMinBufferSize(): %d Hz is not a supported sample rate.", sampleRateInHz);
        return ERROR_BAD_VALUE;
    }

    int size = native_get_min_buff_size(sampleRateInHz, channelCount, audioFormat);
    if ((size == -1) || (size == 0)) {
        ALOGE(TAG, "getMinBufferSize(): error querying hardware");
        return ERROR;
    }
    else {
        return size;
...
static void audioCallback(int event, void* user, void *info)
{
    if (event == android::AudioTrack::EVENT_MORE_DATA) {
        ...
        if (user) {
            ALOGV("ElAudioTrack", "audioCallback event = %d", event);
...
{
    ALOGV(TAG, "sampleRate=%d, audioFormat =%d, channel mask=%x, buffSize=%d",
    ...
    if (android::AudioSystem::getOutputFrameCount(&afFrameCount, streamType) != android::NO_ERROR) {
        ALOGE(TAG, "Error creating AudioTrack: Could not get AudioSystem frame count.");
        return AUDIOTRACK_ERROR_SETUP_AUDIOSYSTEM;
    }
    if (android::AudioSystem::getOutputSamplingRate(&afSampleRate, streamType) != android::NO_ERROR) {
        ALOGE(TAG, "Error creating AudioTrack: Could not get AudioSystem sampling rate.");
        return AUDIOTRACK_ERROR_SETUP_AUDIOSYSTEM;
    }
    ...
    if (!audio_is_output_channel(nativeChannelMask)) {
        ALOGE(TAG, "Error creating AudioTrack: invalid channel mask.");
    ...
    case AUDIO_STREAM_DTMF:
        ...
        break;
    default:
        ALOGE(TAG, "Error creating AudioTrack: unknown stream type.");
    ...
    if ((audioFormat != AudioFormat::ENCODING_PCM_16BIT) &&
        (audioFormat != AudioFormat::ENCODING_PCM_8BIT)) {
        ALOGE(TAG, "Error creating AudioTrack: unsupported audio format.");
    ...
    if ((audioFormat == AudioFormat::ENCODING_PCM_8BIT)
        && (mode == MODE_STATIC)) {
        ALOGV(TAG, "android_media_AudioTrack_native_setup(): requesting MODE_STATIC for 8bit \
    ...
    if (nSession == NULL) {
        ALOGE(TAG, "Error creating AudioTrack: invalid session ID pointer");
    ...
    if (lpTrack == NULL) {
        ALOGE(TAG, "Error creating uninitialized AudioTrack");
    ...
    if (mode == MODE_STREAM) {
        ...
        if (!lpStorage->AllocSharedMem(buffSizeInBytes)) {
            ALOGE(TAG, "Error creating AudioTrack in static mode: error creating mem heap base");
    ...
    if (lpTrack->initCheck() != android::NO_ERROR) {
        ALOGE(TAG, "Error initializing AudioTrack");
        goto native_init_failure;
    }
...
void ElAudioTrack::native_pause()
{
    assert(mNativeAudioTrack);
...
int ElAudioTrack::native_write_byte(void* audioData, int offsetInBytes, int sizeInBytes, int format)
{
    assert(mNativeAudioTrack);
    if (audioData == NULL) {
        ALOGE(TAG, "NULL array of audio data to play, can't play");
