...
public:
    ...
    WTF::String textWithoutTranscoding() const;

    InlineTextBox* createInlineTextBox();
    void dirtyLineBoxes(bool fullLayout);

    virtual void absoluteRects(WTF::Vector<IntRect>&, int tx, int ty);
    void absoluteRectsForRange(WTF::Vector<IntRect>&, unsigned startOffset = 0, unsigned endOffset = UINT_MAX, bool useSelectionHeight = false);

    virtual void absoluteQuads(WTF::Vector<FloatQuad>&);
    void absoluteQuadsForRange(WTF::Vector<FloatQuad>&, unsigned startOffset = 0, unsigned endOffset = UINT_MAX, bool useSelectionHeight = false);

    enum ClippingOption { NoClipping, ClipToEllipsis };
    void absoluteQuads(WTF::Vector<FloatQuad>&, ClippingOption option = NoClipping);
    ...
    virtual float width(unsigned from, unsigned len, const Font&, float xPos, WTF::HashSet<const SimpleFontData*>* fallbackFonts = 0, GlyphOverflow* = 0) const;
    virtual float width(unsigned from, unsigned len, float xPos, bool firstLine = false, WTF::HashSet<const SimpleFontData*>* fallbackFonts = 0, GlyphOverflow* = 0) const;
...
private:
    void computePreferredLogicalWidths(float leadWidth, WTF::HashSet<const SimpleFontData*>& fallbackFonts, GlyphOverflow&);
    ...
    float widthFromCache(const Font&, int start, int len, float xPos, WTF::HashSet<const SimpleFontData*>* fallbackFonts, GlyphOverflow*) const;
    bool isAllASCII() const { return m_isAllASCII; }
    void updateNeedsTranscoding();

    inline void transformText(WTF::String&) const;
