...
typedef WTF::Vector<RefPtr<RenderStyle>, 4> PseudoStyleCache;
...
#if ENABLE(DASHBOARD_SUPPORT)
    WTF::Vector<StyleDashboardRegion> dashboardRegions() const { return rareNonInheritedData->m_dashboardRegions; }
    void setDashboardRegions(WTF::Vector<StyleDashboardRegion> regions) { SET_VAR(rareNonInheritedData, m_dashboardRegions, regions); }
...
#if ENABLE(DASHBOARD_SUPPORT)
    static const WTF::Vector<StyleDashboardRegion>& initialDashboardRegions();
    static const WTF::Vector<StyleDashboardRegion>& noneDashboardRegions();
#endif
