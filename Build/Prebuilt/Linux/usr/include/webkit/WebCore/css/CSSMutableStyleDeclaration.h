...
public:
    ...
    static PassRefPtr<CSSMutableStyleDeclaration> create(const WTF::Vector<CSSProperty>& properties)
    {
    ...
    virtual WTF::String cssText() const;
    virtual void setCssText(const WTF::String&, ExceptionCode&);

    virtual unsigned virtualLength() const;
    unsigned length() const { return m_properties.size(); }

    virtual WTF::String item(unsigned index) const;

    virtual PassRefPtr<CSSValue> getPropertyCSSValue(int propertyID) const;
    virtual WTF::String getPropertyValue(int propertyID) const;
    virtual bool getPropertyPriority(int propertyID) const;
    virtual int getPropertyShorthand(int propertyID) const;
    virtual bool isPropertyImplicit(int propertyID) const;

    virtual void setProperty(int propertyId, const WTF::String& value, bool important, ExceptionCode&);
    virtual WTF::String removeProperty(int propertyID, ExceptionCode&);
    ...
    bool setProperty(int propertyID, const WTF::String& value, bool important = false, bool notifyChanged = true);

    WTF::String removeProperty(int propertyID, bool notifyChanged = true, bool returnText = false);

    // setLengthProperty treats integers as pixels! (Needed for conversion of HTML attributes.)
    void setLengthProperty(int propertyId, const WTF::String& value, bool important, bool multiLength = false);
    void setStringProperty(int propertyId, const WTF::String& value, CSSPrimitiveValue::UnitTypes, bool important = false); // parsed string value
    void setImageProperty(int propertyId, const WTF::String& url, bool important = false);

    // The following parses an entire new style declaration.
    void parseDeclaration(const WTF::String& styleDeclaration);
...
private:
    ...
    CSSMutableStyleDeclaration(CSSRule* parentRule, const WTF::Vector<CSSProperty>&);
    ...
    WTF::String getShorthandValue(const int* properties, size_t) const;
    WTF::String getCommonValue(const int* properties, size_t) const;
    WTF::String getLayeredShorthandValue(const int* properties, size_t) const;
    WTF::String get4Values(const int* properties) const;
    WTF::String borderSpacingValue(const int properties[2]) const;

    template<size_t size> WTF::String getShorthandValue(const int (&properties)[size]) const { return getShorthandValue(properties, size); }
    template<size_t size> WTF::String getCommonValue(const int (&properties)[size]) const { return getCommonValue(properties, size); }
    template<size_t size> WTF::String getLayeredShorthandValue(const int (&properties)[size]) const { return getLayeredShorthandValue(properties, size); }
    ...
    WTF::Vector<CSSProperty>::const_iterator findPropertyWithId(int propertyId) const;
    WTF::Vector<CSSProperty>::iterator findPropertyWithId(int propertyId);

    WTF::Vector<CSSProperty, 4> m_properties;
