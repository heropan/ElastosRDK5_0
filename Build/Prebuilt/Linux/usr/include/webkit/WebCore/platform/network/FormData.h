...
public:
    ...
    explicit FormDataElement(const WTF::Vector<char>& array) : m_type(data), m_data(array) { }

#if ENABLE(BLOB)
    FormDataElement(const WTF::String& filename, long long fileStart, long long fileLength, double expectedFileModificationTime, bool shouldGenerateFile) : m_type(encodedFile), m_filename(filename), m_fileStart(fileStart), m_fileLength(fileLength), m_expectedFileModificationTime(expectedFileModificationTime), m_shouldGenerateFile(shouldGenerateFile) { }
    explicit FormDataElement(const KURL& blobURL) : m_type(encodedBlob), m_blobURL(blobURL) { }
#else
    FormDataElement(const WTF::String& filename, bool shouldGenerateFile) : m_type(encodedFile), m_filename(filename), m_shouldGenerateFile(shouldGenerateFile) { }
#endif
    ...
    KURL m_blobURL;
#endif
    WTF::String m_generatedFilename;
...
public:
    ...
    static PassRefPtr<FormData> create(const WTF::Vector<char>&);
    ...
    void appendFile(const WTF::String& filePath, bool shouldGenerateFile = false);
#if ENABLE(BLOB)
    void appendFileRange(const WTF::String& filename, long long start, long long length, double expectedModificationTime, bool shouldGenerateFile = false);
    void appendBlob(const KURL& blobURL);
#endif

    void flatten(WTF::Vector<char>&) const; // omits files
    WTF::String flattenToString() const; // omits files

    bool isEmpty() const { return m_elements.isEmpty(); }
    const WTF::Vector<FormDataElement>& elements() const { return m_elements; }
    const WTF::Vector<char>& boundary() const { return m_boundary; }
...
private:
    ...
    WTF::Vector<FormDataElement> m_elements;

    int64_t m_identifier;
    bool m_hasGeneratedFiles;
    bool m_alwaysStream;
    WTF::Vector<char> m_boundary;
};
