...
namespace WebCore {
    ...
    struct KeypressCommand {
        KeypressCommand() { }
        KeypressCommand(const WTF::String& commandName) : commandName(commandName) { ASSERT(isASCIILower(commandName[0U])); }
        KeypressCommand(const WTF::String& commandName, const WTF::String& text) : commandName(commandName), text(text) { ASSERT(commandName == "insertText:"); }

        WTF::String commandName; // Actually, a selector name - it may have a trailing colon, and a name that can be different from an editor command name.
        WTF::String text;
    };
#endif
    ...
    public:
        ...
        static PassRefPtr<KeyboardEvent> create(const AtomicString& type, bool canBubble, bool cancelable, AbstractView* view,
            const WTF::String& keyIdentifier, unsigned keyLocation,
        ...
        bool getModifierState(const WTF::String& keyIdentifier) const;
