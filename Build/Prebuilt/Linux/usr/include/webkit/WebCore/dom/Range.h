...
public:
    ...
    WTF::String toString(ExceptionCode&) const;

    WTF::String toHTML() const;
    WTF::String text() const;

    PassRefPtr<DocumentFragment> createContextualFragment(const WTF::String& html, ExceptionCode&) const;
    ...
    void textRects(WTF::Vector<IntRect>&, bool useSelectionHeight = false);
    // Transform-friendly
    void textQuads(WTF::Vector<FloatQuad>&, bool useSelectionHeight = false) const;
    void getBorderAndTextQuads(WTF::Vector<FloatQuad>&) const;
    ...
    // for details.
    void expand(const WTF::String&, ExceptionCode&);
...
private:
    ...
    static void processNodes(ActionType, WTF::Vector<RefPtr<Node> >&, PassRefPtr<Node> oldContainer, PassRefPtr<Node> newContainer, ExceptionCode&);
