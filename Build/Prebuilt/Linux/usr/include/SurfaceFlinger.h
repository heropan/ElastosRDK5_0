...
private:
...
#if ACTIONS_FEATURE_DEBUG_FPS_DEFAULT_ON
    //ActionsCode(wh, NEW_FEATURE : add for debug fps)
    ...
    //ActionsCode(wh, NEW_FEATURE : support hw rotation)
#if ACTIONS_FEATURE_HW_ROTATION_DEFAULT_ON
public:
    ...
    struct PropertiesState {
        PropertiesState()
            : mHwRotation(0),
            mDefaultRotation(0),
            mHdmiDefaultFormat(PIXEL_FORMAT_RGB_565)
        { }

        // for phyical panel rotation info
        int mHwRotation; 
        // for logcal display init rotation      
        int mDefaultRotation;
        // for hdmi format config      
        int mHdmiDefaultFormat;
    };
    PropertiesState sPropertiesState;
#endif
};
